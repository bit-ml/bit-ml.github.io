...
Meetups are shown at **Community -> Meetups** (`/community/meetups/`) as an embedded **Google Calendar**.

### For organizers

Add or edit events in the team Google Calendar (ask an admin for edit access). The embed updates automatically; no website rebuild is required.

### For admins (embed URL)

In Google Calendar -> calendar settings -> **Integrate calendar**, copy the **Embed code** `src` URL into `config.toml`:

```toml
[params.meetups]
  embed_src = "https://calendar.google.com/calendar/embed?src=..."
```

Commit and push `config.toml` only when the calendar ID or timezone changes.
